# ------------------------------------------------------------------------------
#
# Estimate swept area (where missing in OSPAR and FlexFile data) of DATRAS HH 
# data using information in HH, FlexFile and OSPAR data
#
#
# WKABSENS / June 2021 
...
for(GEAR in GEARS){
  ...
  if(nrow(model_data) > 0){
    ...
    if(sum(!is.na(model_data$SweepLngt))>0){
      if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))> 2){
        door.norm1   <- glmmTMB(DoorSpread ~ Depth + SweepLngt + (1|Survey) + (1|Ship), model_data, family = gaussian())
        ...
        door.norm1   <- glmmTMB(DoorSpread ~ Depth + SweepLngt + (1|Survey), model_data, family = gaussian())
        print(rmse1 <- RMSE(predict(door.norm1, newdata = model_data, allow.new.levels = T), model_data$DoorSpread))
      } else  {
      ...
      if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))> 2){
        door.lnorm1   <- glmmTMB(logDoorSpread ~ Depth + SweepLngt + (1|Survey) + (1|Ship), model_data, family = gaussian())
        print(rmse2 <- RMSE(exp(predict(door.lnorm1, newdata = model_data, allow.new.levels = T)), model_data$DoorSpread))
      } else if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))<= 2){
        door.lnorm1   <- glmmTMB(logDoorSpread ~ Depth + SweepLngt + (1|Survey), model_data, family = gaussian())
        print(rmse2 <- RMSE(exp(predict(door.lnorm1, newdata = model_data, allow.new.levels = T)), model_data$DoorSpread))
      } else  {
    ...
    if(sum(!is.na(model_data$Warplngt))>0){
      # warplength + depth
      if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))> 2){
        door_warp.norm1   <- glmmTMB(DoorSpread ~ Depth + Warplngt + (1|Survey) + (1|Ship), model_data, family = gaussian())
        print(rmse3 <- RMSE(predict(door_warp.norm1, newdata = model_data, allow.new.levels = T), model_data$DoorSpread))
      } else if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))<= 2){
        door_warp.norm1   <- glmmTMB(DoorSpread ~ Depth + Warplngt + (1|Survey), model_data, family = gaussian())
        print(rmse3 <- RMSE(predict(door_warp.norm1, newdata = model_data, allow.new.levels = T), model_data$DoorSpread))
      } else  {
      ...
      if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))> 2){
        door_warp.lnorm1   <- glmmTMB(logDoorSpread ~ Depth + Warplngt + (1|Survey) + (1|Ship), model_data, family = gaussian())
        print(rmse4 <- RMSE(exp(predict(door_warp.lnorm1, newdata = model_data, allow.new.levels = T)), model_data$DoorSpread))
      } else if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))<= 2){
        door_warp.lnorm1   <- glmmTMB(logDoorSpread ~ Depth + Warplngt + (1|Survey), model_data, family = gaussian())
        print(rmse4 <- RMSE(exp(predict(door_warp.lnorm1, newdata = model_data, allow.new.levels = T)), model_data$DoorSpread))
      } else  {
    ...
    # depth
    if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))> 2){
      door_depth.norm1   <- glmmTMB(DoorSpread ~ Depth + (1|Survey) + (1|Ship), model_data, family = gaussian())
      print(rmse5 <- RMSE((predict(door_depth.norm1, newdata = model_data, allow.new.levels = T)), model_data$DoorSpread))
    } else if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))<= 2){
      door_depth.norm1   <- glmmTMB(DoorSpread ~ Depth + (1|Survey), model_data, family = gaussian())
      print(rmse5 <- RMSE((predict(door_depth.norm1, newdata = model_data, allow.new.levels = T)), model_data$DoorSpread))
    } else  {
    ...
    if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))> 2){
      door_depth.lnorm1   <- glmmTMB(logDoorSpread ~ Depth + (1|Survey) + (1|Ship), model_data, family = gaussian())
      print(rmse6 <- RMSE(exp(predict(door_depth.lnorm1, newdata = model_data, allow.new.levels = T)), model_data$DoorSpread))
    } else if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))<= 2){
      door_depth.lnorm1   <- glmmTMB(logDoorSpread ~ Depth + (1|Survey), model_data, family = gaussian())
      print(rmse6 <- RMSE(exp(predict(door_depth.lnorm1, newdata = model_data, allow.new.levels = T)), model_data$DoorSpread))
    } else  {
  ...
  if(nrow(model_data) > 0){
    ...
    if(sum(!is.na(model_data$SweepLngt))>0){
      if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))> 2){
        wing.norm1   <- glmmTMB(WingSpread ~ Depth + SweepLngt + (1|Survey) + (1|Ship), model_data, family = gaussian())
        print(wing_rmse1 <- RMSE(predict(wing.norm1, newdata = model_data, allow.new.levels = T), model_data$WingSpread))
      } else if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))<= 2){
        wing.norm1   <- glmmTMB(WingSpread ~ Depth + SweepLngt + (1|Survey), model_data, family = gaussian())
        print(wing_rmse1 <- RMSE(predict(wing.norm1, newdata = model_data, allow.new.levels = T), model_data$WingSpread))
      } else  {
      ...
      if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))> 2){
        wing.lnorm1   <- glmmTMB(logWingSpread ~ Depth + SweepLngt + (1|Survey) + (1|Ship), model_data, family = gaussian())
        print(wing_rmse2 <- RMSE(exp(predict(wing.lnorm1, newdata = model_data, allow.new.levels = T)), model_data$WingSpread))
      } else if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))<= 2){
        wing.lnorm1   <- glmmTMB(logWingSpread ~ Depth + SweepLngt + (1|Survey), model_data, family = gaussian())
        print(wing_rmse2 <- RMSE(exp(predict(wing.lnorm1, newdata = model_data, allow.new.levels = T)), model_data$WingSpread))
      } else  {
    ...
    if(sum(!is.na(model_data$Warplngt))>0){
      if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))> 2){
        wing_warp.norm1   <- glmmTMB(WingSpread ~ Depth + Warplngt + (1|Survey) + (1|Ship), model_data, family = gaussian())
        print(wing_rmse3 <- RMSE(predict(wing_warp.norm1, newdata = model_data, allow.new.levels = T), model_data$WingSpread))
      } else if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))<= 2){
        wing_warp.norm1   <- glmmTMB(WingSpread ~ Depth + Warplngt + (1|Survey), model_data, family = gaussian())
        print(wing_rmse3 <- RMSE(predict(wing_warp.norm1, newdata = model_data, allow.new.levels = T), model_data$WingSpread))
      } else  {
      ...
      if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))> 2){
        wing_warp.lnorm1   <- glmmTMB(logWingSpread ~ Depth + Warplngt + (1|Survey) + (1|Ship), model_data, family = gaussian())
        print(wing_rmse4 <- RMSE(exp(predict(wing_warp.lnorm1, newdata = model_data, allow.new.levels = T)), model_data$WingSpread))
      } else if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))<= 2){
        wing_warp.lnorm1   <- glmmTMB(logWingSpread ~ Depth + Warplngt + (1|Survey), model_data, family = gaussian())
        print(wing_rmse4 <- RMSE(exp(predict(wing_warp.lnorm1, newdata = model_data, allow.new.levels = T)), model_data$WingSpread))
      } else  {
    ...
    # depth
    if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))> 2){
      wing_depth.norm1   <- glmmTMB(WingSpread ~ Depth + (1|Survey) + (1|Ship), model_data, family = gaussian())
      print(wing_rmse5 <- RMSE((predict(wing_depth.norm1, newdata = model_data, allow.new.levels = T)), model_data$WingSpread))
    } else if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))<= 2){
      wing_depth.norm1   <- glmmTMB(WingSpread ~ Depth + (1|Survey), model_data, family = gaussian())
      print(wing_rmse5 <- RMSE((predict(wing_depth.norm1, newdata = model_data, allow.new.levels = T)), model_data$WingSpread))
    } else  {
    ...
    if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))> 2){
      wing_depth.lnorm1   <- glmmTMB(logWingSpread ~ Depth + (1|Survey) + (1|Ship), model_data, family = gaussian())
      print(wing_rmse6 <- RMSE(exp(predict(wing_depth.lnorm1, newdata = model_data, allow.new.levels = T)), model_data$WingSpread))
    } else if(length(unique(model_data$Survey))> 2 & length(unique(model_data$Ship))<= 2){
      wing_depth.lnorm1   <- glmmTMB(logWingSpread ~ Depth + (1|Survey), model_data, family = gaussian())
      print(wing_rmse6 <- RMSE(exp(predict(wing_depth.lnorm1, newdata = model_data, allow.new.levels = T)), model_data$WingSpread))
    } else  {
...
hh$Distance[idx] <- 1000 * apply(cbind(hh$ShootLong[idx],hh$ShootLat[idx],hh$HaulLong[idx],hh$HaulLat[idx]),1,
                                function(x)spDistsN1(matrix(x[1:2], ncol = 2),matrix(x[3:4], ncol = 2), longlat = T))
